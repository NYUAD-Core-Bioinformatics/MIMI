...
Usage::
    ...
    mimi_mass_analysis [-h] -p PPM -vp VPPM -c DBBINARY [DBBINARY ...] -s SAMPLE [SAMPLE ...] -o OUTPUT [-g]

Arguments:
  -p PPM, --ppm PPM     Parts per million for mono isotopic mass matching
  -vp VPPM              Parts per million for isotope verification
  -c DBBINARY           Binary DB input file(s)
  -s SAMPLE             Input sample file(s)
...
mimi_cache_create
---------------
Create precomputed cache files for molecular mass data.

Usage::

    mimi_cache_create [-h] [-l JSON] -d DBTSV [DBTSV ...] -i {pos,neg} -c DBBINARY [-g]

Arguments:
  -l JSON, --label JSON     Labeled atoms configuration file
  -d DBTSV, --dbfile DBTSV  File(s) with list of compounds
  -i {pos,neg}, --ion {pos,neg}  Ionisation mode
  -c DBBINARY, --cache DBBINARY  Binary DB output file
  -g, --debug               Enable debug output
...
mimi_hmdb_extract
--------------
Extract metabolite information from HMDB database.
...
Arguments:
  ...
  -l MIN_MASS, --min-mass MIN_MASS  Lower bound of molecular weight in Da
  -u MAX_MASS, --max-mass MAX_MASS  Upper bound of molecular weight in Da
  -o OUTPUT, --output OUTPUT  Output TSV file path (default: metabolites.tsv)
...
mimi_kegg_extract
--------------
Extract compound information from KEGG database.
...
Arguments:
  -l MIN_MASS, --min-mass MIN_MASS  Lower bound of molecular weight in Da
  -u MAX_MASS, --max-mass MAX_MASS  Upper bound of molecular weight in Da
  -i COMPOUND_IDS, --input COMPOUND_IDS  Input TSV file containing KEGG compound IDs
  -o OUTPUT, --output OUTPUT  Output TSV file path (default: kegg_compounds.tsv)
...
Options:
  -n NUM_COMPOUNDS, --num-compounds NUM_COMPOUNDS  Number of compounds to output
  -i NUM_ISOTOPES, --num-isotopes NUM_ISOTOPES  Number of isotopes per compound
